...
class IsoConfig(BaseSettings):
    ...
    # File Paths — override via ISO_LOGS_DIR, ISO_STAGING_DIR, etc.
    logs_dir: Path = Path("logs")
    staging_dir: Path = Path("staging")
    cache_db: Path = Path("data/isomutator_cache.db")
    ledger_file: Path = Path("vulnerabilities.jsonl")
    ...
    def create_directories(self) -> 'IsoConfig':
